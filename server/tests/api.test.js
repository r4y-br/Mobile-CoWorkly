...
describe('Rate Limiter', () => {
    // Create a separate app instance with rate limiter for testing
    let rateLimitedApp;
    
    beforeAll(async () => {
        const { limiter } = await import('../lib/rateLimiter.js');
    ...
    test('should block requests after exceeding the limit', async () => {
        // Create a fresh app with a very low limit for testing
        ...
        const blockedRes = await request(testApp).get('/limited');
        expect(blockedRes.statusCode).toBe(429);
        expect(blockedRes.body).toHaveProperty('error', 'Too many requests');
    });

    test('should have correct configuration values', async () => {
        const { limiter } = await import('../lib/rateLimiter.js');
